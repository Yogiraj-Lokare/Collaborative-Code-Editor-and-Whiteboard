...
//const { socket, peerjsEndpoint } = env_config;
import { socket } from "../../../socket/socket";

const peerjsEndpoint = 3001;
...
class SocketConnection {
  ...
  constructor(settings: any) {
    ...
    this.socket = socket; //initializeSocketConnection();
  ...
  connectToNewUser(userData: any, stream: MediaStream) {
    ...
    call.on("stream", (userVideoStream: MediaStream) => {
      console.log("receving stream...", userVideoStream);
  ...
  createVideo = (createObj: CreateVideo) => {
    if (!this.videoContainer[createObj.id]) {
      ...
      let elem = document.getElementById(createObj.id);
      //@ts-ignore
      elem.srcObject = createObj.stream;
    }
  };
